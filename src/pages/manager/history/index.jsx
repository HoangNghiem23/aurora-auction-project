import {
  ...
  function HistoryAuction() {
    ...
    const handleViewHistory = async (auction) => {
      try {
        const response = await api.get(`/bid/HistoryOfBid?auctionId=${auction.id}`);
    ...
    const bidHistoryColumns = [
      { title: "Amount of Money", dataIndex: "amountofmoney", key: "amountofmoney" },
    ...
    return (
      <div>
        ...
        <Modal
          ...
          footer={null}
        >
          <Table dataSource={bidHistory} columns={bidHistoryColumns} rowKey="id" />
        </Modal>
      </div>
    );
  }
